...
Description: OggChat is a unique live chat software solution that integrates directly with Gmail, Google Talk, and Google Apps and lets you interact directly with website visitors right from your preferred Instant Messenger on your desktop, iPhone, or Android mobile phone.
Version: 1.4.2
Author: OggChat
...
load_plugin_textdomain('wp-content/plugins/oggchat-live-chat-software');
...
function oggchat_plugin_actions($links, $file) {
    ...
    if($file == $this_plugin && function_exists('admin_url')) {
        $settings_link = '<a href="'.admin_url('options-general.php?page=oggchat-live-chat').'">'.__('Settings').'</a>';
...
<div class="wrap">
        <?php screen_icon() ?>
    <h2><?php _e('OggChat Live Chat') ?></h2>
    <div class="metabox-holder meta-box-sortables ui-sortable pointer">
        <div class="postbox" style="float:left;width:30em;margin-right:10px">
            <h3 class="hndle"><span><?php _e('OggChat Chat Widget Settings') ?></span></h3>
            <div class="inside" style="padding: 0 10px">
                <form method="post" action="options.php">
                    <p style="text-align:center"><?php wp_nonce_field('update-options') ?><a href="http://www.oggchat.com/" title="<?php _e('Unique live chat software that lets you chat right from Gmail, Google Talk, or your Mobile Phone') ?>"><img src="<?php echo($plugurldir) ?>oggchat.png" height="74" width="227" alt="<?php _e('OggChat Live Chat') ?>" /></a></p>

                    <p><label for="oggchatID"><?php printf(__('Enter your %1$sIf you don\'t have an account, click here to learn more about OggChat%2$sOggChat%3$s chat key below to activate the plugin.'), '<strong><a href="http://www.oggchat.com/" title="', '">', '</a></strong>') ?></label><br /><input type="text" name="oggchatID" id="oggchatID" value="<?php echo(get_option('oggchatID')) ?>" style="width:100%" /><small class="nonessential"><?php _e('<br>Your Chat Key is located at OggChat.com on your Member Dashboard.') ?></small></p>
                    <p>
...
<option value="bottom" <?php if (get_option('oggchatLocation') == 'bottom') {printf('selected');}?> > Bottom </option>
                    ...
                    <p class="submit" style="padding:0"><input type="hidden" name="action" value="update" />
                        ...
                        <input type="submit" name="oggchatSubmit" id="oggchatSubmit" value="<?php _e('Save Settings') ?>" class="button-primary" /> </p>
                    <p style="font-size:smaller;color:#999239;background-color:#ffffe0;padding:0.4em 0.6em !important;border:1px solid #e6db55;-moz-border-radius:3px;-khtml-border-radius:3px;-webkit-border-radius:3px;border-radius:3px"><?php printf(__('Don&rsquo;t have an OggChat account? %1$sRegister now for a free trial!%2$sRegister now for a Free Trial!%3$s'), '<a href="https://oggchat3.icoa.com/signup.jsp" title="', '">', '</a>') ?></p>			</form>
            </div>
        </div>
        <div class="postbox" style="float:left;width:38em">
            <h3 class="hndle"><span><?php _e('Overview') ?></span></h3>
            <div class="inside" style="padding: 0 10px">			<p><?php printf(__('<b>Why Try Oggchat?</b><br>OggChat is a unique live chat solution that supports Gmail, Google Talk, iPhone, Android, and BlackBerry.  For more information on customizing your chat widget and details on OggChat, please visit %1$sLive Chat and Live Help Software for your website%2$sOggChat.com%3$s.'), '<a href="http://www.oggchat.com/" title="', '">', '</a>') ?></p>

                <p><?php printf(__('<b>Would you like to use your own chat buttons instead of a chat tab?</b><br>Choose from our list of buttons or upload you own.  %1$sClick here for an overview of including OggChat in WordPress%2$sClick here to see how%3$s.'), '<a href="http://oggchat.com/index.php/live-chat-software-features/faqs/10-wordpress-steps-for-oggchat" title="', '">', '</a>') ?></p>
            </div>
        </div>
    </div>
</div>
    <?php }
    add_submenu_page('options-general.php', __('OggChat Settings'), __('OggChat Settings'), 'manage_options', 'oggchat-live-chat', 'oggchat_settings_page');
}
